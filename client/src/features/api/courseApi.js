...
const REVIEW_API = "http://localhost:3000/api/v1/review";
  ...
  endpoints: (builder) => ({
    ...
    createReview: builder.mutation({
      query: ({ courseId, rating, comment }) => ({
        url: `${REVIEW_API}/create`,
        method: "POST",
        body: { courseId, rating, comment },
      }),
    }),
    getReviewsForCourse: builder.query({
      query: (courseId) => ({
        url: `${REVIEW_API}/${courseId}`,
        method: "GET",
      }),
    }),
    updateReview: builder.mutation({
      query: ({ reviewId, rating, comment }) => ({
        url: `${REVIEW_API}/${reviewId}`,
        method: "PUT",
        body: { rating, comment },
      }),
    }),
